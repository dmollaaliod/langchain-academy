...
def call_model(state: State):
    ...
    # If no summary exists, we just use a default system message    
    else:
        system_message = f"You are a helpful assistant."
        past_messages = state.get("messages", "")
        if not past_messages:
            past_messages = [HumanMessage(content="Hello")]

        messages = [SystemMessage(content=system_message)] + past_messages
